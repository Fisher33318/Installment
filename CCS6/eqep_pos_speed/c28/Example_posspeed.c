...
//
//    theta_mech = QPOSCNT/mech_Scaler = QPOSCNT/2000, where 2000 is the number of
//                 counts in 1 revolution.(2000/4 = 500 line/rev. quadrature encoder)
//
// 2. This program calculates: **theta_elec**
//
//    theta_elec = (# pole pairs) * theta_mech = 2*QPOSCNT/2000 for this example
//
// 3. This program calculates: **SpeedRpm_fr**
//
//    SpeedRpm_fr = [(x2-x1)/2000]/T                                             - Equation 1
//    Note (x2-x1) = difference in number of QPOSCNT counts. Dividing (x2-x1) by
//    2000 gives position relative to Index in one revolution.
// If base RPM  = 6000 rpm:   6000 rpm = [(x2-x1)/2000]/10ms                     - Equation 2
//                                     = [(x2-x1)/2000]/(.01s*1 min/60 sec)
//                                     = [(x2-x1)/2000]/(1/6000) min
//                         max (x2-x1) = 2000 counts, or 1 revolution in 10 ms
//
//
// If both sides of Equation 2 are divided by 6000 rpm, then:
//                            1 = [(x2-x1)/2000] rev./[(1/6000) min * 6000rpm]
//							Because (x2-x1) must be <2000 (max) for QPOSCNT increment,
//						    (x2-x1)/2000 < 1 for CW rotation
//                          And because (x2-x1) must be >-2000 for QPOSCNT decrement,
//                          (x2-x1)/2000>-1  for CCW rotation
//						    speed_fr = [(x2-x1)/2000]/[(1/6000) min * 6000rpm]
//                                   = (x2-x1)/2000                              - Equation 3
//
// To convert speed_fr to RPM, multiply Equation 3 by 6000 rpm
//                           SpeedRpm_fr = 6000rpm *(x2-x1)/2000                 - Final Equation
//
//
...
//    where X = QCAPCTL [UPPS]/2000 rev. (position relative to Index in 1 revolution)
// If max/base speed = 6000 rpm:  6000 = (32/2000)/[(t2-t1)/(150MHz/128)]
//    where 32 = QCAPCTL [UPPS] (Unit timeout - once every 32 edges)
//          32/2000 = position in 1 revolution (position as a fraction of 1 revolution)
//          t2-t1/(150MHz/128),  t2-t1= # of QCAPCLK cycles, and
//		                  1 QCAPCLK cycle = 1/(150MHz/128)
//										  = QCPRDLAT
//
//		        So: 6000 rpm = [32(150MHz/128)*60s/min]/[2000(t2-t1)]
//		             t2-t1 = [32(150MHz/128)*60 s/min]/(2000*6000rpm)           - Equation 5
//		                   = 94 CAPCLK cycles = maximum (t2-t1) = SpeedScaler
//
// Divide both sides by (t2-t1), and:
//                   1 = 94/(t2-t1) = [32(150MHz/128)*60 s/min]/(2000*6000rpm)]/(t2-t1)
...
//
//					 speed_pr = 94/(t2-t1)
//                      or [32(150MHz/128)*60 s/min]/(2000*6000rpm)]/(t2-t1)  - Equation 6
//
// To convert speed_pr to RPM:
// Multiply Equation 6 by 6000rpm:
//                  SpeedRpm_fr  = 6000rpm * [32(150MHz/128)*60 s/min]/[2000*6000rpm*(t2-t1)]
//							                = [32(150MHz/128)*60 s/min]/[2000*(t2-t1)]
//                                        or [(32/2000)rev * 60 s/min]/[(t2-t1)(QCPRDLAT)]- Final Equation
//
//
...
// The value for freqScaler_pr becomes: [32*(100MHz/128)*60s/min]/(2000*6000rpm) = 63
...
void POSSPEED_Calc_1(POSSPEED *p)
{
     long tmp;
...
//**** Position calculation - mechanical and electrical motor angle  ****//
     ...
     // where mech_scaler = 2000 cnts/revolution
...
// Unit Timer is configured for 100Hz in INIT function
    ...
    {
        /** Differentiator  **/
        // The following lines calculate position = (x2-x1)/2000 (position in 1 revolution)
...
void POSSPEED_Calc_2(POSSPEED *p)
{
     long tmp;
     unsigned int pos16bval,temp1;
     _iq Tmp1,newp,oldp;
...
void  POSSPEED_Init_3(void)
{
    ...
    EQep3Regs.QUPRD=1500000;         // Unit Timer for 100Hz at 150 MHz SYSCLKOUT
...
void POSSPEED_Calc_3(POSSPEED *p)
{
     long tmp;
     unsigned int pos16bval,temp1;
     _iq Tmp1,newp,oldp;
...
//**** Position calculation - mechanical and electrical motor angle  ****//
     p->DirectionQep = EQep3Regs.QEPSTS.bit.QDF;    // Motor direction: 0=CCW/reverse, 1=CW/forward

     pos16bval=(unsigned int)EQep3Regs.QPOSCNT;     // capture position once per QA/QB period
     p->theta_raw = pos16bval+ p->cal_angle;        // raw theta = current pos. + ang. offset from QA
...
// Check an index occurrence
     ...
     {
        p->index_sync_flag = 0x00F0;
        EQep3Regs.QCLR.bit.IEL=1;                   // Clear interrupt flag
     }
...
// Unit Timer is configured for 100Hz in INIT function
    ...
    if(EQep3Regs.QFLG.bit.UTO==1)                    // If unit timeout (one 100Hz period)
    {
        ...
        EQep3Regs.QCLR.bit.UTO=1;                   // Clear interrupt flag
    }
